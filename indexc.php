...
<div id="raam">
    ...
    foreach($kategooriad as $k){
		...
		echo '<script>
			$sisudM['.$k['id'].']='.json_encode($sisud).';
		</script>';			
					
        foreach($sisud as $s){
            echo "<div class='sisu'>";
                    echo "<button type='button' class='modal-nupp' data-toggle='modal' data-target='#viewModal' data-kategid='".$k['id']."' data-postituseid='".$s['id']."'>".$s['pealkiri']."</button>";
		...
		<div class="modal-dialog">
			...
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title"><?= $xml->pealkiri->$keel ?></h4>
				</div>
				<div class="modal-body" id="modalkeha">
					<div id="post-vasak" style="float:left">
					<p class="modal-sisu"><?= $xml->sisu->$keel ?></p>
					</div>
					<div id="post-parem" style="float:right">
						<div id="modal-kommentaarid">
							
							
						</div>
						<div id="modal-kommentaar">
							<div class="form-group">
								<label for="modal-kommentaar2"><?= $xml->kommentaar->$keel ?></label>
								<textarea class="form-control" rows="5" id="modal-kommentaar2" title=<?= $xml->kommentaar->$keel ?>></textarea>
							</div>
							<button type="button" class="v-nupp" id="postitakommentaar" data-postid=""><?= $xml->esita->$keel ?></button>
						</div>
					</div>
				</div>
				<div class="modal-footer">
					<button type="button" class="v-nupp" data-dismiss="modal"><?= $xml->sule->$keel ?></button>
				</div>
			</div>
